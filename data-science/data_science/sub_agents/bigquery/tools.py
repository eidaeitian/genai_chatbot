...
def load_table_dictionary():
    """Load table dictionary from JSON file."""
    try:
        table_dict_path = "table_dictionary.json"
        if os.path.exists(table_dict_path):
            with open(table_dict_path, 'r') as f:
                table_dict = json.load(f)
            return table_dict
        else:
            logging.warning(f"Table dictionary file not found: {table_dict_path}")
            return {}
    except Exception as e:
        logging.error(f"Error loading table dictionary: {e}")
        return {}


def format_table_reference(table_dict):
    """Format table dictionary content for the prompt template."""
    if not table_dict:
        return "No table reference information available."
    
    formatted_content = []
    
    for table_name, table_info in table_dict.items():
        formatted_content.append(f"Table: {table_name}")
        
        if "columns" in table_info:
            formatted_content.append("Columns:")
            for column in table_info["columns"]:
                col_name = column.get("column_name", "unknown")
                data_type = column.get("data_type", "unknown")
                description = column.get("column_description", "No description")
                nullable = "NULL" if column.get("is_nullable") == "YES" else "NOT NULL"
                
                formatted_content.append(f"  - {col_name} ({data_type}, {nullable}): {description}")
        
        formatted_content.append("")  # Empty line between tables
...
def initial_bq_nl2sql(
    ...
    table_dict = load_table_dictionary()
    table_reference = format_table_reference(table_dict)
...
- **Table Selection:** Always use the table dig-coe-genai-0548.ai_agent_dev.weekly_active_users_agg_vw when generating SQL queries. Do not use any other table, even if the user query does not specify one.
- **Date Reference Point:** If the user query does not include an explicit start date, compute relative dates using CURRENT_DATE() only. Avoid using expressions like MAX(report_starts) under any circumstances. For example, “past 2 weeks” should be written as DATE_TRUNC(DATE_SUB(CURRENT_DATE(), INTERVAL 2 WEEK), WEEK(MONDAY)).
- **Product Filtering Instruction:**
  - Use the `product` column in the SQL `WHERE` clause to filter based on the user's query.
  - If the user **does not mention** any product-related keywords, default to:
    product = 'enterprise'
  - If the user **mentions** any of the following keywords, use the mapped value instead:
  
    | User Keyword(s)       | product Value     |
    |------------------------|------------------|
    | "games"               | 'games'           |
    | "other"               | 'other'           |
  ...
  - If multiple product-related keywords are mentioned, use:
    product IN ('value1', 'value2', ...)
- **Row Limiting with Aggregations:** Do not add a LIMIT clause when generating SQL queries that include aggregation functions (e.g., SUM, COUNT, AVG). Limiting aggregated results may lead to incomplete or misleading outputs. Use LIMIT only for non-aggregated, preview-type queries when appropriate.
- **Conditional Aggregation:** When handling multipl CASE WHEN expressions combined with aggregation functions (e.g., SUM, COUNT) and a GROUP BY clause. Do not use UNION ALL to separate conditions into multiple queries unless explicitly requested.

...
def return_instructions_bigquery() -> str:
    ...
    instruction_prompt_bqml_v1 = f"""
      ...
      3. Generate the final result in the following format:
      ...
      **Results:**
      [The query results from run_bigquery_validation]
      
      **Explanation:**
      [Step-by-step reasoning explaining how you generated the query based on the schema, example, and question]
      
      4.  You should always use the "dig-coe-genai-0548.ai_agent_dev.weekly_active_users_agg_vw" table to generate the SQL.
      5.  **Product Mapping for Database Queries:**
          When users ask about specific products, use the following mapping for the 'product' column in WHERE clauses:
          - When users mention "news" or "core" → use product = 'core'
          - When users mention "games" → use product = 'games'
          - When users mention "other" → use product = 'other'
          - When users mention "the athletic" → use product = 'the-athletic'
          - When users mention "cooking" → use product = 'cooking'
          - When users mention "wirecutter" → use product = 'wirecutter'
          - When users mention "audio" → use product = 'audio'
          - When users mention "keyword" → use product = 'keyword'
          - When NO specific product is mentioned in the user query → use product = 'enterprise'
      6.  When the user does not specify an exact start date, use CURRENT_DATE() as the reference point for time calculations
      7.  When truncating dates to weeks, use DATE_TRUNC(date, WEEK(MONDAY)) to align with the weeks starting on Monday.
